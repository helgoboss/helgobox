...
impl RealTimeProcessor {
    ...
    pub fn process_incoming_midi_from_vst(
        ...
        if self.midi_control_input == MidiControlInput::FxInput {
            ...
            if transport_is_starting && msg.r#type() == ShortMessageType::NoteOff {
                ...
                self.process_unmatched_short(msg, Caller::Vst);
                return;
            }
            self.process_incoming_midi(frame_offset, msg, Caller::Vst);
        } else {
    ...
    /// Always returns false if on monitoring FX chain because not relevant in that case for our
    /// logic.
    fn is_currently_playing(&self) -> bool {
