...
An _absolute_ control value is a percentage between 0.0% and 100.0% (internally represented by a high-precision floating point number between 0.0 and 1.0).

[[relative-control-value]]
===== Relative control value

A _relative_ control value is a positive or negative integer that reflects the number of increments or decrements.
