...
Source- or target-specific UI user interface elements are described in <<available-source-types>> and <<available-target-types>>.
...
* <<unit-tags>> assigned to this ReaLearn <<unit>>.
...
All <<source>> types share the following common settings and functions:
...
Virtual:: Invocations of <<virtual-control-element,virtual control elements>> (coming from the <<controller-compartment>>).
This source category is available in the <<main-compartment>> only.
...
*** Set the value count of "Parameter 1" to 100 (see <<compartment-parameter>>)
...
All values are entered comma-separated and using the unit chosen in the <<target-section>>.
...
This only works with sources that are capable of displaying text: That is any <<category-osc,OSC source>> with argument type _String_, <<display-source, MIDI Display>> and <<midi-script-source>>.
...
----
** ... which can then be processed by a <<midi-script-source>> source (this example is not realistic, it just shows how you can access the value table returned by the glue section feedback script):
+
[source,lua]
----
return {
...
This source fires whenever one of ReaLearn's <<compartment-parameter>> is changed.
...
In most cases, it's a hardware element on your controller.

Examples: Knobs, encoders, faders, buttons, keys, pads, pitch wheels.

[[control]]
=== Control

In ReaLearn, the word _control_ usually refers to the process of controlling something in REAPER (e.g. triggering an action or adjusting a parameter).

[[feedback]]
=== Feedback

In ReaLearn, the word _feedback_ usually refers to the process of controlling LEDs, motor faders or displays on your device, in response to some events in REAPER (e.g. a track volume change).

==== Distinction of control elements with regard to what they emit
...
A <<control-element>> is absolute if it emits absolute values.
...
====
Fader:: When moving a fader up from bottom to top, it will emit values from 0% to 100%.

Knob:: When moving a knob clockwise, it will emit values from 0% (at the left boundary) to 100% (at the right boundary).

Momentary button:: When pressing a button, it will emit 100%.
When released, it will emit 0%.

Key:: When pressing a key, it will emit a value lower or equal to 100%, depending on the velocity with which the key was pressed.
====

[[relative-control-element]]
===== Relative control element

A <<control-element>> is relative if it emits relative values.
You can think of a relative value as an instruction, either the instruction "Go to the previous value!" or the instruction "Go to the next value!".
The first instruction is called *decrement*, the second one *increment*.

.Typical relative control elements
====
Rotary endless encoders:: When moving a rotary endless encoder clockwise, it will continuously emit increments.
When moving it counter-clockwise, it will continuously emit decrements.
+
It happens very often that controllers have rotary endless encoders, but they will act like knobs by default, sending absolute messages.
That is a great waste and you should change that setting as soon as possible on the hardware side. **Relative control is always superior to absolute control!**
====

==== Distinction of control elements with regard to whether they are real
...
Virtual control elements make it possible to create <<main-preset,main presets>>
...
That's why we sometimes use the term <<control-element>> even if we also want to talk about the corresponding <<feedback-element>>.

[[input-port]]
=== Input port

For letting you control things, ReaLearn needs to somehow listen to events coming from your <<controller>>.
...
For sending feedback back to your controller, ReaLearn needs to somehow send events back to your <<controller>>.
It does so by talking to an output port, which can be a MIDI output device port or an OSC port.
...
A _source_ is part of a <<mapping>> and describes in most cases a control and/or feedback element on a controller.
...
A _glue_ is part of a <<mapping>> that sits between the source and the target and filters or transforms control and feedback streams.
...
An absolute control value is a percentage between 0.0% and 100.0% (internally represented by a high-precision floating point number between 0.0 and 1.0).

A relative control value is a positive or negative integer that reflects the number of increments or decrements.
E.g. control value -2 means a decrement of 2.

[[feedback-value]]
=== Feedback value

A _feedback value_ is the signal that travels through a ReaLearn <<mapping>> back from <<target>> to <<source>> when a <<target>> changes its value.
...
=== Use the controller compartment!
