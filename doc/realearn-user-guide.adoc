...
This section explains important concept used in ReaLearn.
...
The main compartment lets you define what the controller device should do.
...
You can achieve this by setting the unit key of each volume-controlling ReaLearn unit to exactly the same value, in each project, using <<set-unit-data>>.
...
[#auto-load]
=== Auto-load main presets

If you activate auto-load mode _Based on unit FX_, ReaLearn will start to observe the <<unit-fx>> of this ReaLearn unit.
...
Also see <<best-practice-output-device>>.
...
* *Export session as JSON:* Copies a _complete_ dump of ReaLearn's current settings (including all mappings, even controller mappings) to the clipboard in JSON format.
...
Factory:: Contains available <<factory-compartment-presets>>.

User (...):: Contains available <<user-compartment-presets>>.
Multiple of such submenus may exist.
Each one represents a preset namespace (or workspace).
...
This button is only available for the main compartment because auto-load is only about loading main presets.

===== Mapping group controls
...
This allows you to change the group name and change things that affect all mappings in this group: Assigning tags, enabling/disabling control and/or feedback, setting an activation condition.
...
In the center you can see the info area:
...
[qanda]
[[best-practice-output-device]] Prefer output to a specific device over <FX output>!::
