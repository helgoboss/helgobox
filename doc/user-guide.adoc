...
|===
|Last update of text: |`2021-10-09 (v2.11.0-pre.3)`
...
CAUTION: ReaLearn allows you to spread your text over all available displays and lines. This is great if you need to display a lot of text but one display doesn't provide enough space. But be aware: Replacing feedback with other feedback ("feedback relay") doesn't work so nicely anymore if you make use of this feature.

If you want to know how to define which text shall be sent to the displays, please see <<textual-feedback,textual feedback>>.
...
* *Textual:* By default, ReaLearn sends numeric feedback values to the controller. If you tick this checkbox, you can
send textual feedback values instead. This only works with sources that are capable of displaying text: That is any <<category-osc,OSC source>> with argument type _String_ and the <<display-source, MIDI Display source>>.
* *Feedback transformation (EEL):* This field is available only with numeric feedback. It's like _Control transformation (EEL)_ (see further below) but used for
...
[#textual-feedback]
* *Textual feedback expression:* This field is available only with textual feedback. Here you define which text is going to be sent to the source _whenever the target value changes_ and also - for your convenience - immediately at the moment of entering the text.
...
* *Toggle buttons:* Toggle buttons mode is used to toggle a target between on and off states. It only makes sense for momentary buttons (which fire a value > 0 on each press).
