...
|===
|Last update of text: |`2022-03-25 (v2.13.0-pre.1)`
...
[NOTE]
====
The option _Do not send reset messages_ isn't available in older REAPER versions. If you don't use any external hardware synths, you can untick the global options *Reset on: Play* and *Reset on: Stop* instead!
====
...
+
NOTE: _Enable input for control messages_ is totally irrelevant for ReaLearn. This is
...
* *Auto-name listed mappings:* Clears the names of all listed mappings so ReaLearn's auto-naming mechanism can kick
 in.
* *Make sources of all main mappings virtual:* Attempts to make the sources in the main compartment virtual by matching them with the sources in the controller compartment. This is useful if you already learned a bunch of sources in the main compartment, just to find out later that you would like to have used a controller preset.
* *Make targets of listed mappings sticky:* Changes the targets of all currently listed mappings to use "sticky" object selectors by attempting to resolve the objects from non-sticky selectors. We call object selectors _sticky_ if they refer to a very particular object (e.g. a track).
** Sticky selectors: `<Master>`, `<This>`, `By ID`
** Non-sticky selectors: `<Dynamic>`, `<Focused>`, `<Selected>`, `<All selected>`, `By position`, `By name`, `All by name`
...
* *Reload all presets from disk:* If you made direct changes to preset files or have downloaded presets via ReaPack, you should press this to reflect these changes in the preset lists of all open ReaLearn instances (reloads all preset files).
+
[NOTE]
====
...
====
* [[osc-devices,OSC devices]] *OSC devices:* Allows one to display and modify the list of (globally) configured OSC devices.
...
You can easily find it by pressing the _Projection_ button in ReaLearn and scrolling down a bit. It's the value next to *Host* and should start with `192.168.`.
====
...
* *Log incoming real messages:* When enabled, all incoming MIDI messages, OSC messages or key pressed will be logged to the console. Each log entry contains the following information:
...
* *Log incoming virtual messages:* When enabled, all triggered virtual control elements and their values will be logged (see <<controller-compartment>>).
* *Log outgoing real messages:* When enabled, all outgoing MIDI or OSC messages will be logged to the console. The log entries look similar to the ones described above, with the following notable differences.
** Message purpose
...
* *Log outgoing virtual messages:* When enabled, all feedback events to virtual control elements will be logged (see <<controller-compartment>>).
...
Denotes the argument type which ReaLearn should use to construct a proper feedback message.
...
* The value that's send will be derived from the type (see <<feedback-arguments>>):
+
[cols="m,m"]
|===
| Type | Property
...
TIP: If you hold a key, it will not keep firing! Use the <<fire-mode>> setting instead.

In order to set the key, simply click the *Learn* button and press the key combination of your choice.

TIP: This only works if <<control-input>> is set to *Computer keyboard*.
...
p
|
Array of floating-point numbers between 0.0 and 1.0
|
...
All parameter values are normalized floating point values, that means they are decimal numbers between 0.0 and 1.0.
...
Available in dynamic _track_ selectors only.
...
** Will point to track with index 0 (first track) if "Parameter 1" is set to the minimum and to
 track with index 99 (= track position 100) if it's set to the maximum.
...
** In future, it will probably be possible
...
A target that allows you to define a track. It does nothing really useful at the moment but is suited very well as neutral target for textual feedback with an expression that contains a track property, e.g. `{{ target.track.name }}`.
...
Allows you to come up with a performance control mode typical for synth parameter mappings. Try this
...
suitable preset for your controller available on ReaPack or in the https://github.com/helgoboss/realearn/tree/master/resources/controllers/unofficial[list of unofficial controller presets], build your own.
...
[IMPORTANT]
====
Make sure to watch out for dedicated controller presets on the Helgoboss ReaPack repository and https://github.com/helgoboss/realearn/tree/master/resources/controllers/unofficial[list of unofficial controller presets]!
...
* See <<osc-devices>> if you are unsure where to enter this data.
...
There's one main preset in the Helgoboss ReaPack repository which makes extensive usage of the capabilities of the Mackie Control preset: <<daw-control-preset>>. If you need inspiration or help how to build main presets for the Mackie Control preset, have a look at it!
====
...
It's typically used to display a time, e.g. the time of the current play position. For this use case, simply combine it with the target <<seek-target>>, choose <<textual-feedback,textual feedback>> in the <<glue>> section and enter one of the `*.mcu` placeholders in the text expression field, for example `{{target.position.project_default.mcu}}`.
...
|
This display supports all latin characters. You can use it with all kinds of targets with both numeric and <<textual-feedback,textual feedback>>.
...
If you want to target both lines at once because your text is quite long, you can do that by using the MIDI source <<display-source>> and setting _Line_ to `<Multiline>`. But keep in mind that this will tie your main preset to Mackie Displays. The advantage of making it work with a virtual control element is that it will be compatible with non-Mackie controllers that use the same virtual control element naming.
