...
|===
|Last update of text: |`2021-11-18 (v2.11.0-pre.14)`
...
** *Expression field:* When you choose this option, a text field will appear next to it. This lets you enter a
 mathematical expression whose result should be a _track index_ (the first track in the project has index 0, the master track -1).
 ...
 _track_ selectors, there's also `this_track_index` (which resolves to the zero-based index of the track
 on which this ReaLearn instance is located) and `selected_track_index` (which resolves to the zero-based index
...
[WARNING]
====
*You should not use any other constructs than the ones in these examples!* It's not
