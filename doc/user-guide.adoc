...
|===
|Last update of text: |`2022-11-02 (v2.14.0-pre.8)`
...
TIP: Loading of NKS presets only works if you have the VST2/VST2i version of the corresponding plug-in installed.

Settings:

* *Track/FX:* You must tell the target at which FX slot to load the corresponding plug-in.

NOTE: The best idea is to use FX selector <<by-position>>. Selectors such as <<fx-by-id>> or <<fx-by-name>> are not suited because the target might replace the plug-in with another one, in which the unique FX ID and the FX name can change. Then the target would turn inactive and stop working.
