...
2+|The structure of the user guide has been redistributed into several key parts that are mandatory for understanding:
a|
====
[upperroman]
. Quick start
. <<basics>> (included <<basic-setup-items>>, <<companion-app>>, <<tutorials>>, FAQ e.t.c.)
. <<Back-to-Main-panel>> with sub-parts:
 * <<header-panel>>
 ** <<Menu-listing-overview>> structure (including context menu)
 * <<bottom-panel>>
 * <<common-compartment-settings>>
 * <<controller-compartment>>
 * <<main-compartment>>
 * <<mapping-row>>
. <<mapping-panel>> with sub-parts:
 * <<mapping>>
 * <<conditional-activation>>
 * <<Bottom-section>> & <<Bottom-Help>> info
 * <<Source>> section
 * <<target>> section
 ** <<category-real>> - in a separate part due to the large amount of information
 * <<glue>> section
 * <<Bottom-section>>
. <<category-real-details>> created the hierarchical structure by group type of target
. <<provided-reaper-action>>
. <<advanced-settings-section>> section
====
a|
|===
...
If the button label ends with a number, that number denotes the number of top-level configuration properties set for that mapping. That way you can immediately see if a mapping has advanced settings or not. You can learn more about the available properties in the section <<advanced-settings-section>>.
...
[#back-to-category]
==== Groupes of Category "Real"

[NOTE]
====
This guide part is very important for learning, but consumed the deep describtion. So it`s relocated to the separately <<category-real-details>>.
Using the links from table below "_Grouping of category "Real"_, you can access specific sections of this target options.
For return back by using links: <<back-to-category>>, wich licated near duplicate screenshots from _"Reference (part 3)_".

The description below had trying combine the 62 items list into a hierarchical structure, for ease of learning the types of target. Therefore, it is slightly different from the list state of the current version of Realearn.
====

.Grouping of category "Real":
|===
|<<global-list>> |image:target -global.svg[Global list] |image:target -pot.svg[Pot list] |<<pot-list>> 
|<<project-list>> |image:target -project.svg[Project list] |image:target -send.svg[Send list] |<<send-list>>
.2+|<<track-list>> .2+|image:target -track.svg[Track target list] |image:target -clip.svg[Clip list] |<<clip-list>>
|image:target -midi OSC.svg[MIDI & OSC target list] |<<midi-osc-list>>
|<<fx-list>> |image:target -fx.svg[FX target list] |image:target -realearn.svg[Realearn list] |<<realearn-list>>
|===
...
[#category-real-details]
= Reference (Part 3) "Real" Details:
:imagesdir: ./images/real
...
kbd:[<<Back-to-Top>>] <= kbd:[<<Back-to-Main-panel>>] <= kbd:[<<mapping-panel>>] <= kbd:[<<Target>>] <= kbd:[<<category-real-details>>]

[#project-list]
== Project list:

.Project list, <<back-to-category>>
...
.MIDI/OSC list, <<back-to-category>>
...
This section describes the _Advanced settings_ feature of the mapping panel (see section <<advanced-settings,Advanced settings>>) in more
detail.
