...
+
* If input is set to xref:user-interface/main-panel/inputoutput-section.adoc#fx-input[], it enables this option so that ReaLearn only sends feedback if the track is armed.
...
Please note that this is technically not always possible when input is set to xref:user-interface/main-panel/inputoutput-section.adoc#fx-input[] or output to xref:user-interface/main-panel/inputoutput-section.adoc#fx-output[], when the background project is not running.
