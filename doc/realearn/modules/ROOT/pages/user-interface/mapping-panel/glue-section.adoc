...
This setting can be used with all targets that work with xref:further-concepts/mapping-concepts.adoc#absolute-control-value[absolute control values] (all targets except xref:targets/project-targets/project-invoke-reaper-action.adoc#project-invoke-reaper-action[Project: Invoke REAPER action] with relative xref:targets/project-targets/project-invoke-reaper-action.adoc#invocation-type[]).
...
TIP: If you want to control _other_ mappings only and not _this_ mapping, just pick a target that doesn't have any effect, for example the xref:targets/realearn-targets/realearn-dummy-target.adoc#realearn-dummy-target[].
...
Only a few targets support that, such as xref::targets/project-targets/project-set-tempo.adoc[].
...
.Speed example
====
Let's assume you selected the discrete target xref:targets/fx-targets/fx-browse-presets.adoc#fx-browse-presets[], which is considered discrete because an FX with for example 5 presets has 6 well-defined possible values (including the <no preset> option), there's nothing inbetween.
