...
A ReaLearn instance matches when at least one of its units is tagged with any of the xref:further-concepts/unit-concepts.adoc#unit-tag[unit tags] entered into this field (comma-separated).
...
CAUTION: You enter xref:further-concepts/unit-concepts.adoc#unit-tag[unit tags] here, but it will enable/disable whole xref:key-concepts.adoc#instance[instances]!
