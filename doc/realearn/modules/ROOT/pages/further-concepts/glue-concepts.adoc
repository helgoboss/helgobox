...
This only works with sources that are capable of displaying text: That is any xref:sources/osc.adoc[] with argument type _String_, xref:sources/midi/display.adoc[MIDI Display] and xref:sources/midi/midi-script.adoc#midi-script-source[].
...
** ... or anything else (`true`, `false` or an arbitrary table ... at the moment, this is only useful for the xref:sources/midi/midi-script.adoc#midi-script-source[] source because other sources don't know how to deal with it)
...
+...+ which can then be processed by a xref:sources/midi/midi-script.adoc#midi-script-source[] source:
