...
xref:sources/midi/raw-midi-sysex.adoc#raw-midi-source[] and xref:target-types/midi-targets/midi-send-message-target.adoc#midi-send-message-target[] allow to enter so-called MIDI patterns, an advanced ReaLearn concept.
