...
TIP: For this kind of use cases you should consider the new xref:targets/realearn/enable-disable-mappings.adoc[] target, which is available since ReaLearn 2.11.0 as an alternative.
...
* *By enabling/disabling other ReaLearn instances:* You can use one main ReaLearn instance containing a bunch of mappings with xref:targets/fx/enable-disable.adoc[] target in order to enable/disable other ReaLearn FX instances.
Then each of the other ReaLearn instances acts as one mapping bank/group.
* *By switching between presets of another ReaLearn instance:* You can use one main ReaLearn instance containing a mapping with xref:targets/fx/browse-presets.adoc[] target in order to browse presets of another ReaLearn FX instance.
