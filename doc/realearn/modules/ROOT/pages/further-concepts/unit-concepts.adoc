...
The unit track can be changed via target xref:targets/track/track.adoc#track-target[].
...
The unit FX can be changed via target xref:targets/fx/fx.adoc#fx-target[].
...
Tags are important if you want to dynamically enable or disable instances using the xref:targets/realearn/enable-disable-instances.adoc#realearn-enable-disable-instances[] target.
...
** *Target output:* A message sent because of either the xref:targets/midi/send-message.adoc#midi-send-message-target[MIDI: Send message] or xref:targets/osc/send-message.adoc#osc-send-message[OSC: Send message] target.
