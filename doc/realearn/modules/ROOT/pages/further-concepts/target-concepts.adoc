...
* The xref:targets/fx-parameter/set-value.adoc[] target is an exception.
...
Read more about
xref:further-concepts/mapping-concepts.adoc#conditional-activation[conditional activation] in the section about the xref:user-interface/mapping-panel.adoc[].