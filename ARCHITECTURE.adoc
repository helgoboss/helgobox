...
- AssemblyScript: interesting because TypeScript-like and operator overloading, but ultimately looks too hard to embed because it needs a WASM runtime and I need to make it itself run within that runtime
...
There are some interesting Lua-like languages out there that seek to address Lua's pain points:

* Teal: Statically typed Lua dialect transpiled to Lua
* Luau: Lua fork with optional static typing

I looked into both and Luau felt more mature and its typing more sophisticated and suited to our use case (especially the structural typing). Also, it doesn't need to be transpiled because the VM itself interprets Luau. Plus, it has string interpolation! And lots of other goodies that help very much in practice.