...
function shift_or_delete_pressed(on)
    return PartialMapping {
        activation_condition = {
            kind = "Expression",
            condition = "p[2] || p[7]",
...
local delete = delete_pressed(true)
local shift_or_delete = shift_or_delete_pressed()
...
local mappings = {
    name("Scroll up") + button("cursor-up") + feedback_disabled() + turbo() + scroll_vertically(-1),
    name("Scroll down") + button("cursor-down") + feedback_disabled() + turbo() + scroll_vertically(1),
    name("Scroll left") + button("cursor-left") + feedback_disabled() + turbo() + scroll_horizontally(-1),
    name("Scroll right") + button("cursor-right") + feedback_disabled() + turbo() + scroll_horizontally(1),
    name("Shift modifier") + button("shift") + set_param(params.shift.index),
    name("Delete modifier") + button("delete") + set_param(params.delete.index),
    name("Undo") + no_mod + button("undo") + feedback_disabled() + clip_matrix_action("Undo"),
    name("Redo") + shift + button("undo") + feedback_disabled() + clip_matrix_action("Redo"),
    --name("Build scene") + delete + button("record") + clip_matrix_action("BuildScene"),
    --name("Click") + button("click") + reaper_action(40364),
}
...
for col = 0, column_count - 1 do
    for row = 0, row_count - 1 do
        ...
        -- Control
        table.insert(mappings, name("Delete") + group(groups.slot_clear) + feedback_disabled() + delete + slot_button(col, row) + clip_management_action(col, row, "ClearSlot"))
